...
if __name__ == "__main__" : 
    ...
    graph = [-1]*10001
    dq = deque()    
    dq.append(s) # 위치와 움직인 거리
    graph[s] = 0
    while dq :
        val = dq.popleft()
        for i in (val -1, val +1,val+5) :
            if 0<=i <=10000 and graph[i]==-1:
                dq.append(i)
                graph[i] = graph[val]+1
    print(graph[e])
```
...
def dfs(x,y) :
    ...
    if graph[x][y] == max_val :
        ...
        for i in range(4) :
            nx,ny = dx[i]+x, dy[i]+y
            if 0<=nx<n and 0<=ny<n and graph[x][y] < graph[nx][ny] and ch[nx][ny] == 0:
                ch[nx][ny] = 1
                dfs(nx,ny)
                ch[nx][ny] = 0
...
- i, j를 이용해 돌리므로 그 안에 4번 돌리는 변수는 i,j 이외에 변수로 하는 것을 잊지말기
...
if __name__ == "__main__" : 
    ...
    for h in range(max_pos) : # 빗물의 높이 
        ch = [[0]*n for i in range(n)] # 체크 리스트도 초기화 
        cnt = 0 # 0으로 초기화 
        for j in range(n) :
            for k in range(n) :
                if graph[j][k] > h and ch[j][k] == 0 :
                    cnt+=1 
                    ch[j][k] = 1
                    dq.append((j,k))
                    while dq :
